...
from models import ConvLSTM_start, ConvLSTM_seq, correct_Cl, y_norm, area_norm
...
trainable_param = []
trainable_param.extend( model.cl_decoder.parameters() )
trainable_param.extend( model.project_cl.parameters()  )  
pytorch_total_params = sum(p.numel() for p in trainable_param)
...
if model_exist==False: 
  ...
  if mode == 'train': torch.save(model.state_dict(), './clmodel'+str(all_id))
