...
def get_data(num_runs, num_batch, datasets):
  ...
  #    Color = (aseq-3)/2        # normalize C to [-1,1]
      ...
      assert np.all(mid<1) and np.all(mid>0)
      angle_arc = aseq[1:]
      print('original angle', angle_arc)
      rotation = ( 1 - mid)*pi/2 
      angle_arc = (angle_arc + rotation)%(pi/2)
      print('embedded angle', angle_arc)

      Color = - ( 2*(angle_arc)/(pi/2) - 1 )
  ...
  return frac_all, param_all, y_all, area_all, G_list, R_list, e_list, angle_arc
