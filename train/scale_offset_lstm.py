...
#device=host
print('device',device)
model_exist = False #True
frames = 26
num_runs = 1001
total_size = frames*num_runs
seq = 1
G = 4     # G is the number of grains
param_len = 0
time_tag = 1
...
filebase = '../../ML_PF5_case1001_Mt47024_grains4_frames25_anis0.130_seed'
filename = filebase+str(111)+ '_rank0.h5'
...
#frac_plot = (frac_asse[(num_runs-1)*G*frames:(num_runs)*G*frames]).reshape((G,frames), order='F').T
aseq_test = aseq_asse[(num_runs-1)*G:(num_runs)*G]
tip_y = tip_y_asse[(num_runs-1)*frames:(num_runs)*frames]
#print(tip_y_asse[-101:])
# trained dataset need to be randomly selected:
idx =  np.arange(num_runs)
np.random.seed(seed)
np.random.shuffle(idx[:-1])
#idx = np.hstack((rand_idx,idx[-1]))
print(idx)
...
class PrepareData(Dataset):
     ...
     def __init__(self, input_, output_, init):
          if not torch.is_tensor(input_):
              self.input_ = torch.from_numpy(input_).to(device)
          if not torch.is_tensor(output_):
              self.output_ = torch.from_numpy(output_).to(device)
          if not torch.is_tensor(init):
              self.init = torch.from_numpy(init).to(device)
     def __len__(self):
...
# pick a random 
plot_idx = num_test-1  # in test dataset
...
for i in range(pred_frames):
   ...
   # train_dat = torch.from_numpy(np.vstack(frac_out_info,))
    frac_new_vec = tohost(model(todevice(train_dat), todevice(frac_test_ini[[plot_idx],:]),todevice(scaler_lstm[[window+i]])) ) 
    print('timestep ',i)
...
frac_out = frac_out/scaler_lstm[:,np.newaxis] + frac_test_ini[[plot_idx],:]
print(frac_out)
