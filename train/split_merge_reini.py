        ...
        for run in range(size_b):
          ...
          for i in range(args.shape[0]):  ## every i is a subsimulation
           ...
           # print(frac_sliced, seq_dat[run][:,grain_id], df)
            ...
            if i>(args.shape[0]-1)//2: frac_sliced[:,0] += np.ones(size_t) - np.sum( frac_sliced, axis = -1 )
            else: frac_sliced[:,-1] += np.ones(size_t) - np.sum( frac_sliced, axis = -1 )
...
def split_grain_h(param_dat, seq_dat, G, G_all, grain_arg_list, domain_factor):
  ...
  #  Gi = np.arange(G)
    ...
    if G==G_all: 
        ...
        for run in range(size_b):
          ...
          args = grain_arg_list[run]#map_grain_fix(frac_layer, G, G_all)
       ...
       #     elif i>0: left_coors[:,i] = G_all/G*np.cumsum(seq_dat[:,0,:], axis=-1)[:,2*i-1]
        ...
        return new_param, new_seq, np.zeros((new_seq.shape[0],1))
