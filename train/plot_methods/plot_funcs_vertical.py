...
#========================start plotting area, plot ini_field, alpha_true, and field======
    ...
    if plot_flag==True:
      fig = plt.figure(figsize=(1.5*xmax/20,1.2*12*ytop/80))
    ...
    #  ax3.set_title('NN', color=bg_color, fontsize=ft)
      ...
      ax4.set_title(str(int(miss_rate*100))+'%',color=bg_color,fontsize=ft)

      plt.savefig(var + '_grains' + str(G) + '_case' + str(plot_idx)+ '_anis' + str(anis)+'_G'+str("%1.1f"%G0)+'R' +str(Rmax) + '_error'+ str("%d"%int(miss_rate*100)) + '_frame' +str(final) +'.pdf',dpi=600,facecolor="white", bbox_inches='tight')
      plt.close()
