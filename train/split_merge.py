...
def split_grain(param_dat, seq_dat, G, G_all):
    ...
    if G==G_all: 
        ...
        for i in range(expand):
            ...
            darea_sliced -= (frac_sliced>0)*frac_change/(1e-12+frac_sliced)*darea_sliced

            frac_sliced[:,:,-1] = ones - np.sum(frac_sliced[:,:,:-1], axis=-1)
            darea_sliced[:,:,-1] += (frac_sliced[:,:,-1]>0)*( ones - np.sum(frac_sliced, axis=-1) )/(1e-12+frac_sliced[:,:,-1])*darea_sliced[:,:,-1]
