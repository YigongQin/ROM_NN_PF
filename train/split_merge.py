       ...
       # new_y = np.min(y_null, axis = 0)
        ...
        ## first give the first and last data
            if i==0:
                ...
                left_coors_grains[:,:,:BC_l] = left_coors[:,np.newaxis,np.newaxis] + np.cumsum(frac[:new_size_b,:,:], axis=-1)[:,:,:BC_l] - frac[:new_size_b,:,:BC_l]
            elif i==expand-1:
                new_frac[:,:,-BC_l:] = frac[-new_size_b:,:,-BC_l:]
                new_area[:,:,-BC_l:] = area[-new_size_b:,:,-BC_l:] #+ area_coeff*new_frac[:,:,-BC_l:]*( y_null[i,:,:] - new_y )[:,:,np.newaxis]
                left_coors_grains[:,:,-BC_l:] = left_coors[:,np.newaxis,np.newaxis] + np.cumsum(frac[-new_size_b:,:,:], axis=-1)[:,:,-BC_l:] - frac[-new_size_b:,:,-BC_l:]
                ...
                left_coors[:,np.newaxis,np.newaxis] + np.cumsum(frac[new_size_b*i:new_size_b*(i+1),:,:], axis=-1)[:,:,mid] - frac[new_size_b*i:new_size_b*(i+1),:,mid]
