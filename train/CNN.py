...
class LSTM_soft(nn.Module):
    def __init__(self,input_len,output_len,hidden_dim,num_layer):
        ...
        self.project = nn.Linear(17, output_len) # input = [batch, dim] 
        self.encoder = nn.Sequential( 
            nn.Conv1d(1, 2, 5, stride=1, padding=1), 
            nn.ReLU(),
            nn.Conv1d(2, 4, 5, stride=1, padding=1),
            nn.ReLU(),
        
        )
        self.decoder = nn.Sequential(

            nn.ConvTranspose1d(4, 2, 5, stride=1, padding=1, output_padding=0),
            nn.ReLU(),
            nn.ConvTranspose1d(2, 1, 5, stride=1, padding=1, output_padding=0),
            nn.Sigmoid()
        )
    ...
    def forward(self, input_frac, frac_ini):
        ...
        CH2 = self.encoder(input_frac)
        target = self.decoder(CH2)
        #print(target.shape)
        target = self.project(target[:,-1,:])
